 ...
 * standard view with the machine primed and again with it full to capacity,
 * and how much is downloaded. The budget is what a good
...
export const BUDGET = { bootMs: 3000, frameMs: 8, fullMs: 8, bundleKb: 400 };
/** How far a figure may move from the baseline before it is a change: a share, and a slack for the noisy ones. */
const TOLERANCE = { bootMs: [0.35, 250], frameMs: [0.3, 0.6], fullMs: [0.3, 0.6], bundleKb: [0.1, 2] } as const;

interface Figures {
  bootMs: number;
  frameMs: number;
  /** The frame with the machine full to capacity. */
  fullMs: number;
  bundleKb: number;
}
...
test('boots, draws and downloads within budget, and as it did before', async ({ page }, info) => {
  ...
  // the standard view: the machine settled, seen from the look picture's camera
  ...
  // and full to capacity, coins rained over every tier and settled: the most a frame can ever have to draw
  const full = await page.evaluate(async () => {
    const g = window.game!;
    g.fill(g.content().capacity - g.state().live);
    g.step(240);
    return g.measureFrame();
  });
  const now: Figures = {
    ...
    fullMs: Math.round(full * 100) / 100,
    bundleKb: bundle,
  };
  info.annotations.push({ type: 'perf', description: JSON.stringify(now) });
  console.log(
    `perf: boot ${now.bootMs} ms, frame ${now.frameMs} ms, full ${now.fullMs} ms, download ${now.bundleKb} kB`,
  );
  ...
  if (process.env.PERF_UPDATE) {
    ...
    const moved: string[] = [];
    for (const key of ['bootMs', 'frameMs', 'fullMs', 'bundleKb'] as const) {
  ...
  expect(now.fullMs, 'frame at capacity within budget').toBeLessThanOrEqual(BUDGET.fullMs);
